...
struct
  ...
  local
    ...
    fun expr_is_list_backed_in_env struct_tab env struct_env (CVar0 (ident, _)) =
          env_contains env (ident_name ident)
      | expr_is_list_backed_in_env struct_tab env struct_env (CCast0 (_, e, _)) =
          expr_is_list_backed_in_env struct_tab env struct_env e
      | expr_is_list_backed_in_env struct_tab env struct_env (CMember0 (base, field_ident, _, _)) =
          (case expr_struct_name struct_env base of
             SOME struct_name =>
               (case Symtab.lookup struct_tab struct_name of
                  SOME fields =>
                    (case List.find (fn (fname, _) => fname = ident_name field_ident) fields of
                       SOME (_, CPtr _) => true
                     | _ => false)
                | NONE => false)
           | NONE => false)
      | expr_is_list_backed_in_env _ _ _ _ = false
    ...
    fun alias_names_from_expr struct_tab env struct_env (CAssign0 (_, CVar0 (ident, _), rhs, _)) acc =
          let
            val acc' = alias_names_from_expr struct_tab env struct_env rhs acc
          in
            if expr_is_list_backed_in_env struct_tab env struct_env rhs then
              ident_name ident :: acc'
            else
              acc'
          end
      | alias_names_from_expr struct_tab env struct_env (CAssign0 (_, lhs, rhs, _)) acc =
          alias_names_from_expr struct_tab env struct_env rhs
            (alias_names_from_expr struct_tab env struct_env lhs acc)
      | alias_names_from_expr struct_tab env struct_env (CBinary0 (_, l, r, _)) acc =
          alias_names_from_expr struct_tab env struct_env r
            (alias_names_from_expr struct_tab env struct_env l acc)
      | alias_names_from_expr struct_tab env struct_env (CUnary0 (_, e, _)) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | alias_names_from_expr struct_tab env struct_env (CIndex0 (a, i, _)) acc =
          alias_names_from_expr struct_tab env struct_env i
            (alias_names_from_expr struct_tab env struct_env a acc)
      | alias_names_from_expr struct_tab env struct_env (CMember0 (e, _, _, _)) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | alias_names_from_expr struct_tab env struct_env (CCast0 (_, e, _)) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | alias_names_from_expr struct_tab env struct_env (CCall0 (f, args, _)) acc =
          List.foldl (fn (a, ac) => alias_names_from_expr struct_tab env struct_env a ac)
            (alias_names_from_expr struct_tab env struct_env f acc) args
      | alias_names_from_expr struct_tab env struct_env (CComma0 (es, _)) acc =
          List.foldl (fn (e, ac) => alias_names_from_expr struct_tab env struct_env e ac) acc es
      | alias_names_from_expr struct_tab env struct_env (CCond0 (c, t, e, _)) acc =
          alias_names_from_expr struct_tab env struct_env e
            ((case t of Some te => alias_names_from_expr struct_tab env struct_env te | None => I)
              (alias_names_from_expr struct_tab env struct_env c acc))
      | alias_names_from_expr _ _ _ _ acc = acc

    fun alias_names_from_decl struct_tab env struct_env (CDecl0 (_, declarators, _)) acc =
          List.foldl
            (fn (((Some declr, Some (CInitExpr0 (init, _))), _), ac) =>
                  if expr_is_list_backed_in_env struct_tab env struct_env init then
                    declr_name declr :: ac
                  else
                    ac
              | (_, ac) => ac)
            acc declarators
      | alias_names_from_decl _ _ _ _ acc = acc

    fun alias_names_from_item struct_tab env struct_env (CBlockStmt0 stmt) acc =
          alias_names_from_stmt struct_tab env struct_env stmt acc
      | alias_names_from_item struct_tab env struct_env (CBlockDecl0 decl) acc =
          alias_names_from_decl struct_tab env struct_env decl acc
      | alias_names_from_item _ _ _ _ acc = acc

    and alias_names_from_stmt struct_tab env struct_env (CCompound0 (_, items, _)) acc =
          List.foldl (fn (item, ac) => alias_names_from_item struct_tab env struct_env item ac) acc items
      | alias_names_from_stmt struct_tab env struct_env (CExpr0 (Some e, _)) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | alias_names_from_stmt _ _ _ (CExpr0 (None, _)) acc = acc
      | alias_names_from_stmt struct_tab env struct_env (CReturn0 (Some e, _)) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | alias_names_from_stmt _ _ _ (CReturn0 (None, _)) acc = acc
      | alias_names_from_stmt struct_tab env struct_env (CIf0 (c, t, e_opt, _)) acc =
          (case e_opt of
             Some e => alias_names_from_stmt struct_tab env struct_env e
           | None => I)
            (alias_names_from_stmt struct_tab env struct_env t
              (alias_names_from_expr struct_tab env struct_env c acc))
      | alias_names_from_stmt struct_tab env struct_env (CWhile0 (c, b, _, _)) acc =
          alias_names_from_stmt struct_tab env struct_env b
            (alias_names_from_expr struct_tab env struct_env c acc)
      | alias_names_from_stmt struct_tab env struct_env (CFor0 (Left (Some i), c, s, b, _)) acc =
          alias_names_from_stmt struct_tab env struct_env b
            (opt_alias_expr struct_tab env struct_env s
              (opt_alias_expr struct_tab env struct_env c
                (alias_names_from_expr struct_tab env struct_env i acc)))
      | alias_names_from_stmt struct_tab env struct_env (CFor0 (Left None, c, s, b, _)) acc =
          alias_names_from_stmt struct_tab env struct_env b
            (opt_alias_expr struct_tab env struct_env s
              (opt_alias_expr struct_tab env struct_env c acc))
      | alias_names_from_stmt struct_tab env struct_env (CFor0 (Right d, c, s, b, _)) acc =
          alias_names_from_stmt struct_tab env struct_env b
            (opt_alias_expr struct_tab env struct_env s
              (opt_alias_expr struct_tab env struct_env c
                (alias_names_from_decl struct_tab env struct_env d acc)))
      | alias_names_from_stmt struct_tab env struct_env (CSwitch0 (e, s, _)) acc =
          alias_names_from_stmt struct_tab env struct_env s
            (alias_names_from_expr struct_tab env struct_env e acc)
      | alias_names_from_stmt struct_tab env struct_env (CCase0 (e, s, _)) acc =
          alias_names_from_stmt struct_tab env struct_env s
            (alias_names_from_expr struct_tab env struct_env e acc)
      | alias_names_from_stmt struct_tab env struct_env (CDefault0 (s, _)) acc =
          alias_names_from_stmt struct_tab env struct_env s acc
      | alias_names_from_stmt struct_tab env struct_env (CLabel0 (_, s, _, _)) acc =
          alias_names_from_stmt struct_tab env struct_env s acc
      | alias_names_from_stmt _ _ _ _ acc = acc

    and opt_alias_expr struct_tab env struct_env (Some e) acc =
          alias_names_from_expr struct_tab env struct_env e acc
      | opt_alias_expr _ _ _ None acc = acc
  ...
  in
    fun find_list_backed_aliases struct_tab (CFunDef0 (_, declr, _, body, _)) =
      let
        ...
        fun iterate env =
          let
            val added =
              distinct (op =) (alias_names_from_stmt struct_tab env struct_env body [])
  ...
  fun mk_implicit_cast (tm, from_cty, to_cty) =
    ...
    if from_cty = to_cty then
      ...
      if C_Ast_Utils.is_ptr from_cty then
        ...
        in
          if (case from_cty of
                C_Ast_Utils.CPtr C_Ast_Utils.CChar => true
              | C_Ast_Utils.CPtr C_Ast_Utils.CSChar => true
              | _ => false)
          then
            let val list_ty = Isa_Type (\<^type_name>\<open>List.list\<close>, [isa_dummyT])
                val v = Isa_Free ("v__promo", list_ty)
                val nil_term = Const (\<^const_name>\<open>Nil\<close>, list_ty)
                val neq_nil =
                  Isa_Const (\<^const_name>\<open>HOL.Not\<close>, @{typ bool} --> @{typ bool})
                    $ (Isa_Const (\<^const_name>\<open>HOL.eq\<close>, list_ty --> list_ty --> @{typ bool}) $ v $ nil_term)
            in C_Term_Build.mk_bind tm (Term.lambda v (C_Term_Build.mk_literal neq_nil)) end
          else if (case vty of Type (\<^type_name>\<open>List.list\<close>, [_]) => true | _ => false)
          then
            ...
            C_Term_Build.mk_bind tm
              (Term.lambda v
                (C_Term_Build.mk_bind
                  (C_Term_Build.mk_literal (conv $ v))
               ...
               (case cty of
                  C_Ast_Utils.CPtr inner =>
                    let
                      ...
                      val typed_old = mk_implicit_cast (C_Term_Build.mk_literal old_var, C_Ast_Utils.CPtr C_Ast_Utils.CVoid, cty)
  ...
  fun translate_expr _ (CConst0 (CIntConst0 (CInteger0 (n, repr, flags), _))) =
    ...
    | translate_expr tctx (CVar0 (ident, _)) =
        ...
        in case C_Trans_Ctxt.lookup_var tctx name of
           ...
           | SOME (C_Trans_Ctxt.ParamListPtr, var, cty) => (C_Term_Build.mk_literal var, cty)
        ...
        | _ =>
            ...
            | (CSubOp0, C_Ast_Utils.CPtr elem_cty, C_Ast_Utils.CPtr _) =>
                let val isa_ty = C_Ast_Utils.hol_type_of elem_cty
                    ...
                    val diff_const =
                      Type.constraint (lhs_ptr_ty --> rhs_ptr_ty --> @{typ nat} --> diff_raw_ty)
          ...
          let
            ...
            val focused =
              if ptr_is_raw then
                ...
                C_Term_Build.mk_focus_field focus_const base_var
            val loc_expr =
            ...
            fun fold_decls [] tctx' = translate_compound_items tctx' rest
              | fold_decls ((name, init_term, cty, arr_meta, alias_list_backed) :: ds) tctx' =
                  if C_Ast_Utils.is_ptr cty andalso not (Option.isSome arr_meta) then
                    ...
                    in
                      if supports_raw_ptr andalso not (prefer_pointer_alias_storage alias_list_backed init_term) then
                        let
                          val raw_ptr_ty = raw_ptr_local_gref_typ ()
                          val stored_init =
                        ...
                        SOME (kind0, _, cty) =>
                          ...
                          then
                            let val (rhs_term, _) = translate_expr tctx rhs
                                val var = Isa_Free (name, isa_dummyT)
                                val kind = pointer_alias_kind (expr_is_list_backed_array tctx rhs)
                                val tctx' = C_Trans_Ctxt.add_var name kind var cty tctx
  ...
  fun translate_fundef struct_tab enum_tab typedef_tab func_ret_types func_param_types global_consts ctxt
    ...
    let
      ...
      (* Create free variables for each parameter.
         Keep raw-pointer parameters concrete when AutoCorrode already has a
         canonical HOL representation for them; this prevents hidden type
         variables from escaping through helper definitions such as
         const-void*-returning tail calls. Other pointer params remain
         inference-driven to avoid perturbing existing translations. *)
      fun param_value_hol_ty cty =
        (case cty of
           C_Ast_Utils.CPtr C_Ast_Utils.CVoid =>
             (case C_Ast_Utils.cty_to_record_typ (!current_decl_prefix) cty of
                SOME ty => ty
      ...
      val param_vars =
        ListPair.mapEq (fn ((n, cty), list_backed) =>
          let val hol_ty = param_value_hol_ty cty
  ...
  fun process_translation_unit tu lthy =
    let
      ...
      val list_backed_alias_envs =
        List.foldl
          (fn (fdef, tab) =>
            Symtab.update (fundef_name fdef, C_Ast_Utils.find_list_backed_aliases struct_tab fdef) tab)
          Symtab.empty fundefs_raw
             ...
             in
               (case expr_struct_name base of
                  SOME struct_name =>
                    (case Symtab.lookup struct_tab struct_name of
                       SOME fields =>
                         (case List.find (fn (fname, _) => fname = C_Ast_Utils.ident_name field_ident) fields of
                            SOME (_, C_Ast_Utils.CPtr _) => true
                          | _ => false)
                     | NONE => false)
                | NONE => false)
             end
